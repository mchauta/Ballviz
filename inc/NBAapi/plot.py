...
def players_picture(player_id):
    URL = "https://ak-static.cms.nba.com/wp-content/uploads/headshots/nba/latest/260x190/%d.png" %player_id
    file = io.BytesIO(urllib.request.urlopen(URL).read())
    return misc.imread(file)

def grantland_shotchart(shotchart,leagueaverage,season, seasonType, ax=None,short_three=False,fg_range=[-9,9]):
...
#    plt.figure(figsize=figsize,facecolor='white') #(0,0.17,0.57)
    ...
    ax.text(0,-7, season + ' ' + seasonType, color='black',horizontalalignment='center',fontsize=20,fontweight='bold')
    court(ax,outer_lines=False,color='black',lw=4.0,direction='down',short_three=short_three);
    ax.axis('off')
    #nba.plot.zones()
    #s=0.8747731368853422
    s = 0.85
    bins = np.concatenate([[-np.inf],np.linspace( [0],fg_range[1],200),[np.inf]])
    colors = ['#06799F','#FFB400','#FF2800']
    ...
    p = PatchCollection(patches,cmap=cm,alpha=1, linewidths=0.3,edgecolors='white')
